...
if($userName==$myname){$canEdit=true;}else{$canEdit=false;}
...
if (isset($_COOKIE['bookFisbn']))
{
  $bookCode=$_COOKIE['bookFisbn'];
	$path = "/".rawurldecode($bookCode)."/";
 ...
 */
function drawFiles($list, &$manager, $canEdit)
{	
	global $relative;
	global $IMConfig;
...
<?php if ($manager->isValidBase() == false) { drawErrorBase($manager); } 
	...
	<?php drawFiles($list[1], $manager, $canEdit); ?>
