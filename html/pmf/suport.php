...
foreach ($words as $word) {
    if (!empty($word)) {
        ...
        $resposta_words = explode(' ', $resposta);
        foreach ($resposta_words as $resposta_word) {
            //Eliminem tots els tags html
            $resposta_word = strip_tags($resposta_word);
            $resposta_word = unhtmlentities($resposta_word); //utilitzem la funció que hem creat a db.php
