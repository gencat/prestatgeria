 ...
 * @version $Id: CategoryUtil.class.php 27045 2009-10-21 14:27:04Z drak $
...
class CategoryUtil
{
     ...
     * @return The resulting folder object array
     */
    function getCategoriesByRegistry($registry)
    {
        ...
        $where = implode(' OR ', $where);
        $cats = CategoryUtil::getCategories($where, '', 'id');
     ...
     * @return The resulting folder object
     */
    function getCategoriesByParentID($id, $sort = '', $relative = false, $all = false, $assocKey = '', $attributes = null)
    {
        if (!$id) {
            return false;
        }
        ...
        if ($cats && $relative) {
            $category = CategoryUtil::getCategoryByID($id);
            $arraykeys = array_keys($cats);
            foreach ($arraykeys as $key) {
                CategoryUtil::buildRelativePathsForCategory($category, $cats[$key], isset($includeRoot) ? $includeRoot : false);
            }
        }
    ...
    function getParentCategories($id, $assocKey = 'id')
    {
        ...
        do {
            $cat = CategoryUtil::getCategoryByID($cat['parent_id']);
            if ($cat) {
     ...
     * @return The resulting folder object array
     */
    function getCategoriesByPath($apath, $sort = '', $field = 'ipath', $includeLeaf = true, $all = false, $exclPath = '', $assocKey = '', $attributes = null)
    {
        pnModDBInfoLoad('Categories');
        $pntables = pnDBGetTables();
        $category_column = $pntables['categories_category_column'];
     ...
     * @param assocKey     The field to use as the associated array key (optional) (default='')
     * @param attributes   The associative array of attribute field names to filter by (optional) (default=null)
     * @param sortField    The field to sort the resulting category array by (optional) (default='sort_value')
     *
     * @return The resulting folder object array
     */
    function getSubCategories($cid, $recurse = true, $relative = true, $includeRoot = false, $includeLeaf = true, $all = false, $excludeCid = '', $assocKey = '', $attributes = null, $sortField = 'sort_value')
    {
        if (!$cid) {
            return false;
        }
        ...
        $cats = CategoryUtil::getSubCategoriesForCategory($rootCat, $recurse, $relative, $includeRoot, $includeLeaf, $all, $exclCat, $assocKey, $attributes, $sortField);
     ...
     * @return The resulting folder object array
     */
    function getSubCategoriesByPath($apath, $field = 'ipath', $recurse = true, $relative = true, $includeRoot = false, $includeLeaf = true, $all = false, $excludeCid = '', $assocKey = '', $attributes = null, $sortField = 'sort_value')
    {
        if (!$apath) {
            return false;
        }
     ...
     * @return The resulting folder object array
     */
    function getSubCategoriesForCategory($category, $recurse = true, $relative = true, $includeRoot = false, $includeLeaf = true, $all = false, $excludeCat = null, $assocKey = '', $attributes = null, $sortField = 'sort_value')
    {
        if (!$category) {
            return false;
        }
        ...
        if ($recurse) {
            ...
            $cats = CategoryUtil::getCategoriesByPath($ipath, '', 'ipath', $includeLeaf, $all, $ipathExcl, $assocKey, $attributes);
        } else {
            $cats = CategoryUtil::getCategoriesByParentID($category['id'], '', $relative, $all, $assocKey, $attributes);
        ...
        if ($cats && $relative) {
            $arraykeys = array_keys($cats);
            foreach ($arraykeys as $key) {
                CategoryUtil::buildRelativePathsForCategory($category, $cats[$key], $includeRoot);
            }
        }
        ...
        if ($sortField) {
            ...
            $cats = CategoryUtil::sortCategories($cats, $sortField);
        }
     ...
     * @return true or false
     */
    function haveDirectSubcategories($cid, $countOnly = false, $all = true)
    {
        if (!$cid) {
            return false;
        }

        $cats = CategoryUtil::getCategoriesByParentID($cid, '', false, $all);
     ...
     */
    function getCategoryTreeJS($cats, $doReplaceRootCat = true)
    {
     ...
     * @param $entry array
     */
    function _tree_insert(&$tree, $entry, $currentpath = NULL)
    {
        if ($currentpath === NULL) {
        ...
        if (count($pathlist) == 1) {
            ...
            unset($pathlist[0]);
            CategoryUtil::_tree_insert($tree[$root], $entry, implode('/', $pathlist));
        }
    }
    ...
    function _tree_sort($tree, &$cats)
    {
        ...
        foreach ($sorted as $k => $v) {
            CategoryUtil::_tree_sort($tree[$k], $cats);
        }
    }
     ...
     */
    function sortCategories($cats, $sortField = '')
    {
        if (!$cats)
            return $cats;
        ...
        $tree = array();
        foreach ($cats as $c) {
            CategoryUtil::_tree_insert($tree, $c);
        }
        $new_cats = array();
        CategoryUtil::_tree_sort($tree[1], $new_cats);
        return $new_cats;
    }
    ...
    function getCategoryTreeStructure($cats)
    {
        ...
        $cats = CategoryUtil::sortCategories($cats, 'sort_value');
     ...
     * @return The HTML selector code for the given category hierarchy
     */
    function getSelector_Categories($cats, $field = 'id', $selectedValue = '0', $name = 'category[parent_id]', $defaultValue = 0, $defaultText = '', $allValue = 0, $allText = '', $submit = false, $displayPath = false, $doReplaceRootCat = true, $multipleSize = 1, $fieldIsAttribute = false)
    {
        $line = '---------------------------------------------------------------------';
     ...
     */
    function buildRelativePaths($rootCatIDs, &$cats, $includeRoot = false)
    {
        if (!$rootCatIDs) {
            return false;
        }
        ...
        foreach ($cats as $prop => $catID) {
            ...
            $rootCat = CategoryUtil::getCategoryByID($rootCatIDs[$prop]);
            CategoryUtil::buildRelativePathsForCategory($rootCat, $cats[$prop], $includeRoot);
        }
     ...
     * @return The resulting folder path array (which is also altered in place)
     */
    function buildRelativePathsForCategory($rootCategory, &$cat, $includeRoot = false)
    {
        if (!$rootCategory) {
            return false;
        }
        ...
        if (is_numeric($rootCategory)) {
            $rootCategory = CategoryUtil::getCategoryByID($rootCategory);
        }
     ...
     */
    function hasCategoryAccess($categories, $module, $permLevel = ACCESS_OVERVIEW)
    {
