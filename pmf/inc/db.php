...
function getLang($lang){
	...
	if (isset($lang) && !empty($lang)) return $lang;
	return 'ca';
}
...
function ordre($args){
	...
	foreach ($valors as $valor){
		switch($q){
			...
			break;
			case 'p':
				$sql="UPDATE 012_suport SET ordre=$i WHERE sid=".$valor['id'];
			break;
		}
		$i+=10;
		$rs=mysql_query($sql); 
...
//------------Fem una funci� per reempla�ar cadenes tant numeriques com literals------------------s
